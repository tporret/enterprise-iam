...
final class IdpManager {
	...
	public static function sanitize( array $raw ): array|\WP_Error {
		...
		$sanitized = array(
			...
			'saml_sp_certificate'        => sanitize_textarea_field( $raw['saml_sp_certificate'] ?? '' ),
			'saml_sp_private_key'        => sanitize_textarea_field( $raw['saml_sp_private_key'] ?? '' ),
			'saml_authn_requests_signed' => ! empty( $raw['saml_authn_requests_signed'] ),
			'saml_want_assertions_encrypted' => ! empty( $raw['saml_want_assertions_encrypted'] ),
			'saml_want_nameid_encrypted' => ! empty( $raw['saml_want_nameid_encrypted'] ),
			...
			'custom_uid_attr'            => sanitize_text_field( $raw['custom_uid_attr'] ?? '' ),
